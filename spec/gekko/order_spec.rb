...
describe Gekko::Order do
  ...
  describe '#new' do
    it 'should not allow both stop_price and stop_percent to be set simultaneously' do
      expect { Gekko::Order.new(:ask, random_id, random_id, 1, { stop_price: 1, stop_percent: 1 }) }.
        to raise_error('Stop orders must specify exactly one of either stop price, trailing percentage, or trailing offset.')
    end
  end
  ...
  describe '#should_trigger?' do
    it 'should raise an error when called for a non-stop order' do
      expect { Gekko::Order.new(:ask, random_id, random_id, 1).should_trigger?(1) }.
    ...
    it 'should return true for an ask when the last price is below the stop price' do
      expect(Gekko::Order.new(:ask, random_id, random_id, 1, { stop_price: 5_0000_0000 }).should_trigger?(1_0000_0000)).
        to be_truthy
    end

    it 'should return false for an ask when the last price is above the stop price' do
      expect(Gekko::Order.new(:ask, random_id, random_id, 1, { stop_price: 5000_0000 }).should_trigger?(1_0000_0000)).
        to be_falsey
    end

    it 'should return true for a bid when the last price is above the stop price' do
      expect(Gekko::Order.new(:ask, random_id, random_id, 1, { stop_price:  2_0000_0000 }).should_trigger?(1_0000_0000)).
        to be_truthy
    end
