...
describe Gekko::MarketOrder do
  ...
  describe '#new' do
    it 'should reject an ask if it does not specify the size' do
      expect { Gekko::MarketOrder.new(:ask, random_id, random_id, nil, 1_000_0000) }.to raise_error do |err|
    ...
    it 'should reject a bid if it does not specify a quote margin' do
      expect { Gekko::MarketOrder.new(:bid, random_id, random_id, 1_0000_0000, nil) }.to raise_error do |err|
